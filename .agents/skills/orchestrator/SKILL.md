...
2. **Spec each delegated unit** so a Sonnet subagent succeeds without Opus-level judgment: exact **inputs**, the expected **output/artifact**, **acceptance criteria**, and any constraints/conventions to match. Vague spec → wrong result.
   - **Write OBSERVABLE acceptance criteria** — checkable from the artifact + the run's evidence (diff, tool output, test result), never from the agent's word. Full standard + checklist: `docs/network/OBSERVABLE-CRITERIA.md` — apply it to every delegated unit's DoD.
