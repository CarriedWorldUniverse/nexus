...
2. **Spec each delegated unit** so a Sonnet subagent succeeds without Opus-level judgment: exact **inputs**, the expected **output/artifact**, **acceptance criteria**, and any constraints/conventions to match. Vague spec → wrong result.
   - **Write OBSERVABLE acceptance criteria** — checkable from the artifact + the run's evidence alone, never from the agent's word (full standard: `docs/network/OBSERVABLE-CRITERIA.md`). Name the artifact and where it lives; point at ground truth the judge can read (the **diff**, a **tool's output**, a **test result**); require the process's *evidence* not its *claim* (if a tool must produce the result, **name the tool** and require its **output** present — bare assertions get rejected by the provenance gate); make required tokens **literal/verbatim**; never require a value the checker can't independently verify (reframe, or mark **human-verify**). A criterion the gate can't check silently degrades to trusting the narrative — the exact hole the acceptance-gate hardening exists to close.
