...
- **`pull`** = fetch + reconcile each local line against its remote, **then rebase every line onto its parent's new tip** (v0.1.46), re-materializing expressed folders. **`fetch`** = tracking refs only.
- **pull rebases onto the parent, git-rebase style.** A line's own UNPUSHED commits are replayed onto the parent's latest (parents before children; uncommitted edits ride along last) — linear history, no merge commit. A commit that conflicts **stops** the replay: it becomes the working change with its conflicts, `status` shows `rebase: onto main, stopped at "<msg>" (N more to replay)`. Edit the markers out, `cairn resolve <b> <path>`, then **`cairn commit` with no `-m`** (keeps the original message) — the rest replays, stopping again at the next conflict. `cairn undo` rewinds a stop. **Pushed lines are not rebased** (that would need a force-push): they adopt the parent at their next commit, as a merge, as before.
...
12. **Below v0.1.46, `pull` never moved a branch onto its updated parent** — the parent was merged in at your next commit, with a duplicated message and the conflict-marked `(working)` commit left in history (and pushed). Update rather than hand-fixing. **Below v0.1.44, editing during a conflicted pull silently dropped the remote side** (cairn #195). Any sync while the conflict was open (an edit + `status`) rebuilt the merge on its local parent only; `commit` then re-marked the conflict you had just resolved, and `push` was rejected as non-fast-forward. `status` also listed every file the pull brought in as `M`. Recovery on 0.1.44+: `cairn pull` again, resolve, commit. Relatedly, below v0.1.43 a PR that merged main in could show an inflated `ahead` (seen: 28) — update, don't chase it.
