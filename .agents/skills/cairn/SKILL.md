...
- **Working copy:** `init [dir]`, `clone <url> [dir]`, `express <branch> [--from p]`, `unexpress <branch> [--force]`, `commit [branch] -m` (branch inferred inside a folder), `fold <branch> [--force]`, `reparent <branch> <parent>` | `reparent --infer [--dry-run]`, `abandon <branch> [--force]`, `status [branch]`, `diff [branch] | <a> <b>`, `tree`, `ls`, `resolve <branch> <path>`.
