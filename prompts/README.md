...
## Layout
    ...
    central/parts/common.md     policy shared by every identity
    central/parts/aspect.md     policy for long-lived interactive aspects
    central/parts/worker.md     policy for dispatched one-shot runs
    central/parts/scaffold.md   working discipline for models that need it spelled out
    central/composed-*.draft.md generated by compose.sh — the strings that get applied
    aspects/<name>/{nexus,soul,primer}.md   verbatim capture, per aspect
    MANIFEST.json               live versions at time of capture
    compose.sh                  parts -> composed variants
...
|---|---|---|
| audience | `aspect` / `worker` | persists and can be asked, vs one-shot and cannot |
...
fleet is `ornith` (the entire pool), `codex-cli`, `claude-opus-4-7` and
`claude-sonnet-4-6` — and **no Claude 5 model at all**.
...
— delete rules, let the model use judgement, they cut 80% of Claude Code's
system prompt — is calibrated for the Claude 5 generation. Applying it
wholesale here would strip scaffolding the models we actually run still use.

So the rules were **not deleted, they were moved** into `parts/scaffold.md`,
which composes into the scaffolded variant only. When a frontier model runs a
role, serve it lean. When ornith or codex does, serve it scaffolded.

**Nothing selects automatically yet.** `ResolveByName` picks by identity name,
not by model; wiring `Provider`/`Model` into that choice is the follow-up to
NEX-827. Until then the variant is chosen by which file you apply.

**Known gap:** the pool personalities (anvil, harrow, keel, maren, plumb) are
ornith-backed, and if one ever connects as a *base aspect* rather than being
dispatched as a run, it receives the lean aspect text with no scaffold. That
combination is reachable today and has not been exercised.
