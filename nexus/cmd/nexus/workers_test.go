...
func TestListWorkerStatusRows_RealSQLStore(t *testing.T) {
	...
	if rows[0].Agent != "anvil-builder" || rows[0].Role != "builder" || rows[0].Personality != "anvil" || rows[0].WorkItemID != "wi-1" {
		t.Errorf("row = %+v, want agent=anvil-builder role=builder personality=anvil work_item=wi-1", rows[0])
	}
}
...
func TestPrintWorkerStatusTable(t *testing.T) {
	...
	for _, want := range []string{
		...
		"anvil-builder", "builder", "anvil", "wi-1", "running", "3", "1200", "1m30s ago",
...
func TestTruncateCell(t *testing.T) {
	...
	// Long ASCII string — truncated with ellipsis.
	if got := truncateCell("a-very-long-work-item-id", 10); got != "a-very-lo…" {
		t.Errorf("truncateCell(long ASCII) = %q, want truncated with ellipsis", got)
	}

	// Multi-byte UTF-8 string — rune-safe truncation.
	// "🌍🌍🌍🌍🌍🌍🌍🌍" is 8 emoji, each 4 bytes in UTF-8.
	// Truncating to 5 runes should give 4 emoji + ellipsis.
	utf8Long := "🌍🌍🌍🌍🌍🌍🌍🌍"
	if got := truncateCell(utf8Long, 5); got != "🌍🌍🌍🌍…" {
		t.Errorf("truncateCell(long UTF-8) = %q, want 4 emoji + ellipsis", got)
	}

	// Single rune with max=1.
	if got := truncateCell("abc", 1); got != "a" {
		t.Errorf("truncateCell(max=1) = %q, want single rune", got)
	}

	// Empty string.
	if got := truncateCell("", 5); got != "" {
		t.Errorf("truncateCell(empty) = %q, want empty", got)
	}

	// Zero max.
	if got := truncateCell("test", 0); got != "" {
		t.Errorf("truncateCell(max=0) = %q, want empty", got)
	}
}