...
func fixedTS(t *testing.T) time.Time {
	...
	// when tests run with TZ=UTC. Test invokes time.Local fix below.
...
func TestRenderChatFrameInbound(t *testing.T) {
	...
	renderChatFrame(&buf, "plumb", cf, false)
	got := buf.String()
...
func TestNoColorStripsANSI(t *testing.T) {
	...
	renderChatFrame(&withColor, "plumb", cf, true)
	renderChatFrame(&without, "plumb", cf, false)
