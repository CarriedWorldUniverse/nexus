...
**Operator must pre-create this role via Console as <operator-admin-user>** (nexus-cw cannot create IAM roles per the privilege-escalation review).
...
1. **(Operator, <operator-admin-user>)** Pre-create `nexus-cw-instance-role` IAM role + instance profile per §6.
