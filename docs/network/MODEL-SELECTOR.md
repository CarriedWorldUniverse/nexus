...
| **deepseek-v4-pro** · thinking ON ‡ | 2? | 2 | 3 | 3 | 2 | 1 | 2 | metered-prepaid |
...
‡ **`deepseek-v4-pro` — two runs, thinking OFF→ON is decisive.** With thinking *not pinned* it burned 155k tokens then **blocked**. Re-run with `thinking: enabled` + `reasoning_effort: high` pinned (2026-07-07): **10× fewer tokens (15,374), 322s, and passed the gate** — so pinning thinking fixed the block; it was a config artifact, not the `reasoning_content` parser (that's verified sound — see the harness note). BUT the pass is **soft**: v4-pro committed a branch (`builder/NET-66`) yet opened **no PR**, and the acceptance judge marked met=true crediting the *pre-existing* PR #422 (the reference impl already on main). So `Cap=2?` and `Tok=2` are **provisional** — flattered by the task-shape confound (§2 already done) and the weak gate (task_done trusts self-report). A clean number needs a task NOT already implemented on main. Directionally, though: v4-pro with thinking on is fast, cheap-metered, and no longer blocks — promising, unconfirmed.
...
| deepseek-v4-pro — thinking NOT pinned ‡ | ✗ blocked | 155,110 | 1494 | verbose/looping then blocked |
| deepseek-v4-pro — thinking pinned ON ‡ | ~ soft | 15,374 | 322 | 10× fewer tokens, no longer blocks — but SOFT pass (committed branch `builder/NET-66`, opened NO PR; judge credited pre-existing PR #422). Needs a not-already-done task to confirm |
...
4. **DeepSeek v4-flash clears complex — but only in thinking mode**, at ~13× the low-Claude tokens (metered-pennies makes that *affordable* but slow, 1284s, and un-sovereign). The *same* v4-flash in non-thinking mode blocks. So the DeepSeek lesson is **"turn thinking on," not "pick a bigger model."** Confirmed on v4-pro: thinking-off→on flipped it from blocked/155k/1494s to passing/15k/322s (10× fewer tokens) — pinning thinking, not model size, was the fix. (That pass is soft — see ‡ — but the block→pass transition is real.) Routes are now migrated off the deprecating `reasoner`/`chat` names (gone 2026-07-24) with thinking pinned.
