...
## Worker FULLY RUNS the pipeline (2026-07-05) — one documented gap remains
Pushed the real dispatch all the way to a running worker. Provisioned in m1-scratch: 3 builder PVCs (nexus-builder-work/repos, aspect-home-pool.sub-1), claude-oauth secret, a STABLE self-signed broker cert (SANs for the svc DNS) placed at both /certs AND dataDir/tls/server.crt (so the broker sets BrokerCAFile → the worker gets nexus-broker-ca to trust it), CW_NODE_IP=svc-IP. The worker (nexus-builder:dev, agent pool.sub-1) then did EVERYTHING:
- trusted the pinned broker cert (CW_SEAM_CA), booted its hand JWT (parent=pool provider=openai), validated (model=ornith),
- builder home ready, tool policy loaded, **seeded the builder brief from /etc/dispatch/brief.md**,
- **wsclient connected** to wss://broker:7888/connect, **register acknowledged**,
- **started the deliberation goal-loop** and pursued the task.

**The ONE remaining gap (a real derived-worker code bug, newly surfaced):** the goal-loop's LLM call went to `api.openai.com` (401, no key) instead of Ornith. Root cause: `credentials.resolveAspectDefault` does `SELECT default_openai_credential FROM aspects WHERE name=?` with the EXACT name, but the worker validates as the DERIVED `pool.sub-1` which has NO aspect row (only the synthetic `pool` parent does). So the default-credential lookup returns ErrNoDefault → resolveProviderEnv nil → the worker falls back to the process-default openai endpoint. Provisioning an `ornith` provider credential (allowed-aspects=*) did NOT fix it — the miss is the aspect-DEFAULT resolution, not the credential's existence.

**Fix (next session, a real code change — do it properly, not a live hack):** in the credential default resolution (or resolveProviderEnv), strip a derived name to its parent (`pool.sub-1` → `pool`) so a derived worker inherits the parent's `default_openai_credential`; then set `nexus credential aspect-default pool --openai ornith`. Alternative: the claude-code path (worker uses the delivered CLAUDE_CODE_OAUTH_TOKEN, no credential-default needed) but that needs an M1 worker image with the claude CLI (unit 7 added it to deploy/worker/Dockerfile — build nexus-builder:m1). Either gets the worker to a real LLM → full execution.

Everything else is PROVEN end-to-end live: sovereign ledger → orchestrator drain → pool lease → derived-credential mint → k8s Job → worker schedules → connects+authenticates to the broker → receives brief → runs the goal-loop. That is the entire pipeline minus the last LLM hop. Scratch torn down (one-shot). The derived-credential-resolution fix is the single thing between here and a work-item fully executed by a pooled worker.
