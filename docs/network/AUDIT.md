...
5. **lynxai and vessel-voxcpm are NOT aspects — they are services** (operator, confirmed). Mislabeled because everything in the nexus ns looked like an aspect. No persona, no identity, no roster slot → out of the aspect/retirement model entirely. Their destinations differ:
   - **lynxai → core CWB.** It's the ToolRunner's `web_fetch`/`web_extract` backend — infrastructure every agent uses. Move into the `cwb` namespace, ideally as a **cwb-core container** alongside the pillars (mTLS-fronted, porter-backed, reproducible from the cwb-core manifest). Fixes the orphaned `lynxai-env` config in nexus. **Open call:** pillar (own cwb-proto contract like the others) vs supporting service inside cwb-core (internal capability, no public product surface). Recommend **supporting service** — it has no cwb-proto contract and is a backend, not a product. Dropping it silently kills all agent web access, so it moves *before* the nexus ns is cleaned.
   - **vessel-voxcpm → WIP, parked.** TTS, in active development. NOT retiring, NOT relocating in this migration — out of scope. Keeps its NodePort + `voxcpm-cache` PVC. Record: don't delete, don't touch, revisit when ready.
