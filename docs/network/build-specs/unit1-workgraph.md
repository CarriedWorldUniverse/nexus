...
## UNIT 1 DONE 2026-07-05 — gates passed, PR #397
Live e2e GREEN against the real sovereign ledger (independently re-verified: build+vet clean, fake tests pass, TestLiveWorkGraph PASS). Review pass: correct, well-documented; the ledger DoD gate is intentionally bypassed (tickDoD) since gate enforcement = reviewer/security roles + orchestrator, not the tracker checklist. Security: self-asserted issue:admin on the mesh path = the cwb mesh trust model (mTLS cert is the boundary), configurable Scopes field. Minor: ListReady N+1 RPCs; Cancel(requeue) overloads VerdictBlocked for the reason — both acceptable v1. cairn refused a direct fold (main is protected) → pushed line + opened **nexus PR #397** (base main). cwb-proto bumped to the Issue.skills/StatusCategory/Workflow revision; broker+cmd/nexus still green. The verify-gate caught ~9 real-ledger mismatches the fakes missed and the fakes now ENFORCE them.
