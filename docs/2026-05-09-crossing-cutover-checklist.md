...
- **Host:** `agentnetwork.tail41686e.ts.net` (existing tailnet hostname).
...
```
nexus identity init --host agentnetwork.tail41686e.ts.net
```
...
```
nexus cert init --host agentnetwork.tail41686e.ts.net
```
...
The first time, this is the bootstrap path. Open `https://agentnetwork.tail41686e.ts.net:7888/dashboard/` in a browser; the Login overlay should land on the "Register this device" form (because `operator_passkeys` is empty). Complete the WebAuthn ceremony with a label like "little-blue" or "dMon".
...
Verify: `curl -k https://agentnetwork.tail41686e.ts.net:7888/health` returns connection refused. No process named `node` or `comms-mcp.exe` still bound to 7888.

### Step C3 — nexus up

```
NEXUS_OPERATOR_RPID=agentnetwork.tail41686e.ts.net \
NEXUS_OPERATOR_ORIGINS=https://agentnetwork.tail41686e.ts.net:7888 \
NEXUS_DATA_DIR=$NEXUS_DATA_DIR \
...
curl -k https://agentnetwork.tail41686e.ts.net:7888/api/aspects   # JSON roster
```
