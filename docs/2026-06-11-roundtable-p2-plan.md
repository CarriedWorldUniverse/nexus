...
**Key identity decision (locked):** the broker enforces one live session per name, and the parent is awake while its hands run — so hands register under derived names. v1 posts attribute `from: <parent>.sub-N` (truthful audit beats cosmetic attribution; P4's mediation/UI may collapse lineage to the parent visually). Hands never mount the parent's session PVC (RWO, and hands are fresh-context by definition).
